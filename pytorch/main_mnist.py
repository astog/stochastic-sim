    ...
    for batch_idx, (data, target) in enumerate(train_loader, 1):
        ...
        if args.weight_decay > 0:
            for W in model.parameters():
                if bip_reg is None:
                    bip_reg = W.pow(2).sub(1).sum()
                else:
                    bip_reg = bip_reg + W.pow(2).sub(1).sum()
        else:
            bip_reg = 0.0

        # Calculate loss
        loss = criterion(output, target) + (bip_reg * args.weight_decay)
        '''
...
def adam_set_lr(optimizer, epoch):
    print("Pow: Setting LR to:", args.lr_start / pow(epoch, 1.0 / 4.0), end='\n\n')
