...
def to_binary(stoch_s, bipolar=True):
    '''
    params:
    stoch_s -   the stochastic bitstream byte-array
...
def stoch_add(stoch_s1, stoch_s2, bipolar=True, mode=1):
    if mode == 0:
        ...
        return to_stoch(to_binary(stoch_s1, bipolar) + to_binary(stoch_s2, bipolar), stoch_s1.size, bipolar, True)
    elif mode == 2:
...
def binarize_matrix_to_vector(input_matrix, bipolar):
    ...
    for i in xrange(input_matrix.shape[0]):
        result[i] = to_binary(input_matrix[i], bipolar)
...
def binarize_cube_to_matrix(input_cube, bipolar):
    ...
    for row, col in product(xrange(input_cube.shape[0]), xrange(input_cube.shape[1])):
        result[row][col] = to_binary(input_cube[row][col], bipolar)
...
def stoch_dot(input_matrix1, input_matrix2, bipolar, deterministic=False, mode=1):
    ...
    for i in xrange(input_matrix1.shape[0]):
        ...
        # print("{}*{} = {} == {}".format(
        #     to_binary(input_matrix1[i], bipolar), to_binary(input_matrix2[i], bipolar),
        #     to_binary(input_matrix1[i], bipolar) * to_binary(input_matrix2[i], bipolar),
        #     to_binary(pp, bipolar)), end='\n\n')
        print(pp)
        print(result)
        # print("{} + {} == ".format(to_binary(result, bipolar), to_binary(pp, bipolar)), end='\n')
        # result = stoch_add(result, pp, bipolar, mode)
        print(result, end='\n\n')
        # print(to_binary(result, bipolar))
...
if __name__ == '__main__':
    ...
    print("\n=========================\n")
    ap_xyd = to_binary(stoch_dot(xm, ym, bipolar), deterministic)
    print(ap_xyd)
    print(np.dot(xv, yv))
